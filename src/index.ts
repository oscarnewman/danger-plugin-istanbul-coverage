...
---- | ------------: | -----------------: | ----------------: | --------------:`;
  ...
  const lines = Object.keys(coverage.displayed).map((filename) => {
    ...
    const linkFilename = formatLinkName(
...
function getCombinedCoverageCollection(
  ...
  return coveragePaths
    .map((coveragePath) => ({
      parsed: parseSourcePath(coveragePath),
      raw: coveragePath,
    }))
    .map(({ parsed, raw }) =>
      Object.entries(parsed).reduce(
        (acc, [coveragePath, coverageEntry]) => ({
          ...acc,
          [raw.basePath ? path.join(raw.basePath, coveragePath) : coveragePath]:
            coverageEntry,
        }),
        {}
      )
